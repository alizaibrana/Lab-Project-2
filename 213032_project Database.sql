-- Create database
CREATE DATABASE InventoryManagements;
USE InventoryManagements;
...
-- Trigger to update UpdatedAt on changes
...
CREATE TABLE PurchaseOrders (
    ...
    SupplierID INT FOREIGN KEY REFERENCES Suppliers(SupplierID) ON DELETE CASCADE,
...
CREATE TABLE SalesOrders (
    ...
    OrderDate DATETIME DEFAULT GETDATE(),
    Status NVARCHAR(20) CHECK (Status IN ('Pending', 'Shipped', 'Cancelled')) NOT NULL,
    TotalAmount DECIMAL(10, 2) NOT NULL
);
...
CREATE TABLE SalesOrderDetails (
    ...
    SalesOrderID INT FOREIGN KEY REFERENCES SalesOrders(SalesOrderID) ON DELETE CASCADE,
    ProductID INT FOREIGN KEY REFERENCES Products(ProductID) ON DELETE CASCADE,
    Quantity INT NOT NULL,
    UnitPrice DECIMAL(10, 2) NOT NULL
);
...
CREATE TABLE StockMovements (
    ...
    ProductID INT FOREIGN KEY REFERENCES Products(ProductID) ON DELETE CASCADE,
    MovementType NVARCHAR(20) CHECK (MovementType IN ('IN', 'OUT', 'ADJUSTMENT')) NOT NULL,
    Quantity INT NOT NULL,
